...
const ProductScreen = ({ productName, onBackClick, onCartClick, onProfileClick, onLogoClick, onAddToCart, showCart = true }: ProductScreenProps) => {
  ...
  // Функция для использования обработчика скролла в JSX
  const handleScroll = () => {
    if (contentRef.current) {
      const position = contentRef.current.scrollTop;
      setScrollPosition(position);
  ...
  // Функция для определения высоты изображения
  const getImageHeight = () => {
    // Обычная высота в зависимости от устройства (можно настроить)
    return isImageExpanded ? '60vh' : '40vh';
  };
    ...
    <div className="h-full flex flex-col text-white bg-gradient-to-b from-[#1D1816] via-[#2C2320] to-[#1D1816]">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 z-50">
        <button
          ...
          className={`w-full py-3.5 rounded-xl bg-gradient-to-r ${colors.button} text-white font-medium shadow-lg ${colors.shadow} relative overflow-hidden group transition-all active:scale-95 flex items-center justify-center`}
        >
